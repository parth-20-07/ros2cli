...
from ros2pkg.api.create import populate_rust_library
...
class CreateVerb(VerbExtension):
    ...
    def add_arguments(self, parser, cli_name):
        ...
        parser.add_argument(
            "--build-type",
            default="ament_cmake",
            choices=["cmake", "ament_cmake", "ament_python", "ament_cargo"],
            help="The build type to process the package with",
        )
        parser.add_argument(
            "--dependencies", nargs="+", default=[], help="list of dependencies"
        )
        parser.add_argument(
            "--maintainer-email", help="email address of the maintainer of this package"
        ),
        parser.add_argument(
            "--maintainer-name", help="name of the maintainer of this package"
        ),
        parser.add_argument("--node-name", help="name of the empty executable")
        parser.add_argument("--library-name", help="name of the empty library")
    ...
    def main(self, *, args):
        ...
        def get_git_config(key: str) -> Optional[str]:
            ...
            if git is not None:
                result = subprocess.run(
                    [git, "config", key], stdout=subprocess.PIPE, text=True
                )
        ...
        maintainer_name: str = (
            args.maintainer_name or get_git_config("user.name") or getpass.getuser()
        )
        ...
        if args.build_type == "ament_python" and args.package_name == "test":
            ...
            return (
                "Aborted since 'ament_python' packages can't be named 'test'. Please "
        ...
        print(
            "dependencies:", [str(dependency) for dependency in package.build_depends]
        )
        if node_name:
        ...
        if args.build_type == "ament_python":
            ...
            populate_ament_python(
                package, package_directory, source_directory, node_name
            )
            if node_name:
        ...
        if args.build_type == "ament_cmake" or args.build_type == "cmake":
            if node_name:
                if not source_directory:
                    return (
                        "unable to create source folder in "
            ...
            if library_name:
                if not source_directory or not include_directory:
                    return (
                        "unable to create source or include folder in "
                        + args.destination_directory
                    )
                populate_cpp_library(
                    package, source_directory, include_directory, library_name
                )
        ...
        if args.build_type == "ament_cargo":
            ...
            if node_name:
                ...
                populate_rust_node(package, source_directory, node_name)
            if library_name:
                if not source_directory:
                    return (
                        "unable to create source folder in "
                ...
                populate_rust_library(package, source_directory, library_name)
